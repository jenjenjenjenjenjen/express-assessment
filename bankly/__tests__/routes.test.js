...
describe("POST /auth/login", function() {
  ...
  //TESTS BUG #2
  test("should throw error for wrong username/password", async function() {
    const response = await request(app)
      .post("/auth/login")
      .send({
        username: "u1",
...
describe("GET /users", function() {
  ...
  //TESTS BUG #1
  test("should only return username, first_name and last_name of every user", async function() {
    const response = await request(app)
      .get("/users")
      .send({ _token: tokens.u1 });
    expect(response.statusCode).toBe(200);
...
describe("PATCH /users/[username]", function() {
  ...
  test("should patch data if user is updating own profile", async function() {
    const response = await request(app)
      .patch("/users/u1")
      .send({ _token: tokens.u1, first_name: "new-fn1" }); // u1 is logged in user
    expect(response.statusCode).toBe(200);
    expect(response.body.user).toEqual({
      username: "u1",
      first_name: "new-fn1",
